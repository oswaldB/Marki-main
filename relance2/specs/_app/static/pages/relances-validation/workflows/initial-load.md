...
|------------|------------------------|
| `state-initialized` | `console.log('[WORKFLOW.relances-validation-initial-load] STEP: État de sélection initialisé')` |
| `loading-shown` | `console.log('[WORKFLOW.relances-validation-initial-load] START: Affichage skeleton loader')` |
| `auth-verified` | `console.log('[WORKFLOW.relances-validation-initial-load] STEP: Token auth vérifié')` |
| `relances-fetch-start` | `console.log('[WORKFLOW.relances-validation-initial-load] STEP: Appel API GET /api/relances?valide=false&statut=pret%20pour%20envoi')` |
| `relances-fetched` | `console.log('[WORKFLOW.relances-validation-initial-load] DATA: Relances à valider reçues:', {count: relancesAValider.length})` |
| `templates-fetch-start` | `console.log('[WORKFLOW.relances-validation-initial-load] STEP: Appel API GET /api/templates')` |
| `templates-fetched` | `console.log('[WORKFLOW.relances-validation-initial-load] DATA: Templates reçus:', {count: templates.length})` |
| `data-stored` | `console.log('[WORKFLOW.relances-validation-initial-load] STEP: Données stockées dans le store')` |
| `table-rendered` | `console.log('[WORKFLOW.relances-validation-initial-load] STEP: Tableau rendu avec sélection')` |
| `batch-actions-enabled` | `console.log('[WORKFLOW.relances-validation-initial-load] STEP: Boutons d action batch activés')` |
| `loading-complete` | `console.log('[WORKFLOW.relances-validation-initial-load] END: Page validation chargée en', duree, 'ms')` |
| `loading-error` | `console.error('[WORKFLOW.relances-validation-initial-load] ERROR:', error)` |
