...
|------------|------------------------|
| `loading-shown` | `console.log('[WORKFLOW.dashboard-initial-load] START: Affichage spinner chargement')` |
| `auth-verified` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Token auth vérifié')` |
| `impayes-fetch-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Appel API GET /api/impayes?facture_soldee=0&statut=impaye')` |
| `impayes-fetched` | `console.log('[WORKFLOW.dashboard-initial-load] DATA: Impayés reçus:', {count: impayes.length})` |
| `relances-fetch-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Appel API GET /api/relances?date=today')` |
| `relances-fetched` | `console.log('[WORKFLOW.dashboard-initial-load] DATA: Relances reçues:', {count: relances.length})` |
| `events-fetch-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Appel API GET /api/events?limit=10')` |
| `events-fetched` | `console.log('[WORKFLOW.dashboard-initial-load] DATA: Events reçus:', {count: events.length})` |
| `last-sync-fetch-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Appel API GET /api/events?type=sync&limit=1')` |
| `last-sync-fetched` | `console.log('[WORKFLOW.dashboard-initial-load] DATA: Dernière synchro:', lastSyncTime)` |
| `new-invoices-fetch-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Appel API GET /api/impayes (nouvelles factures)')` |
| `new-invoices-fetched` | `console.log('[WORKFLOW.dashboard-initial-load] DATA: Nouvelles factures reçues:', {count: newInvoices.length})` |
| `kpis-calculate-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Début calcul KPIs')` |
| `kpis-calculated` | `console.log('[WORKFLOW.dashboard-initial-load] SUCCESS: KPIs calculés:', kpis)` |
| `chart-calculate-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Calcul données graphique')` |
| `chart-data-calculated` | `console.log('[WORKFLOW.dashboard-initial-load] DATA: Données graphique calculées:', chartData)` |
| `chart-init-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Initialisation Chart.js')` |
| `chart-rendered` | `console.log('[WORKFLOW.dashboard-initial-load] SUCCESS: Graphique rendu')` |
| `top-debiteurs-calculate-start` | `console.log('[WORKFLOW.dashboard-initial-load] STEP: Calcul top débiteurs')` |
| `top-debiteurs-calculated` | `console.log('[WORKFLOW.dashboard-initial-load] DATA: Top débiteurs calculé:', {count: topDebtors.length})` |
| `loading-complete` | `console.log('[WORKFLOW.dashboard-initial-load] END: Dashboard chargé en', duree, 'ms')` |
| `loading-error` | `console.error('[WORKFLOW.dashboard-initial-load] ERROR:', error)` |
