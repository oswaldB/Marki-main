...
|------------|------------------------|
| `start` | `console.log('[WORKFLOW.portail-mission-download-facture] START: Téléchargement facture pour impayé', this.impaye?.id)` |
| `loading-set` | `console.log('[WORKFLOW.portail-mission-download-facture] STEP: loading = true, error = null')` |
| `api-call` | `console.log('[WORKFLOW.portail-mission-download-facture] API_CALL: POST /functions/generatePdfLink', { impayelId: this.impaye?.id })` |
| `pdf-loaded` | `console.log('[WORKFLOW.portail-mission-download-facture] STEP: Lien signé reçu, expiration =', data.data?.expires)` |
| `download-triggered` | `console.log('[WORKFLOW.portail-mission-download-facture] STEP: window.open() appelé sur URL signée')` |
| `end` | `console.log('[WORKFLOW.portail-mission-download-facture] SUCCESS: Téléchargement déclenché en', duree, 'ms')` |
| `error` | `console.error('[WORKFLOW.portail-mission-download-facture] ERROR:', error)` |
