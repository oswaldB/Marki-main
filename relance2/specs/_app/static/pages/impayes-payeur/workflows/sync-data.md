...
|------------|------------------------|
| `start` | `console.log('[WORKFLOW.impayes-payeur-sync-data] START: Synchronisation des données impayés (regroupé par payeur)')` |
| `button-disabled` | `console.log('[WORKFLOW.impayes-payeur-sync-data] STEP: Bouton désactivé, syncing = true')` |
| `api-call-start` | `console.log('[WORKFLOW.impayes-payeur-sync-data] STEP: Appel API POST /api/workflows/import-invoices')` |
| `progress-100` | `console.log('[WORKFLOW.impayes-payeur-sync-data] STEP: syncProgress = 100, lastSyncTime enregistré')` |
| `data-fetched` | `console.log('[WORKFLOW.impayes-payeur-sync-data] DATA: Réponse reçue:', {imported, updated})` |
| `table-updated` | `console.log('[WORKFLOW.impayes-payeur-sync-data] STEP: Table payeurs rechargée et regroupée par contact')` |
| `toast-shown` | `console.log('[WORKFLOW.impayes-payeur-sync-data] STEP: Toast de succès affiché')` |
| `end` | `console.log('[WORKFLOW.impayes-payeur-sync-data] SUCCESS: Synchronisation terminée en', duree, 'ms')` |
| `error` | `console.error('[WORKFLOW.impayes-payeur-sync-data] ERROR:', error)` |
