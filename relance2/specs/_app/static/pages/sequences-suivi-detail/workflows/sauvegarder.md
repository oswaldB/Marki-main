...
|------------|------------------------|
| `start` | `console.log('[WORKFLOW.sequences-suivi-detail-sauvegarder] START: Sauvegarde séquence de suivi', { sequenceId: this.sequence?.id, hasChanges: this.hasChanges })` |
| `validation` | `console.log('[WORKFLOW.sequences-suivi-detail-sauvegarder] STEP: Validation des données', { nom: this.sequence.nom, type_suivi: this.sequence.type_suivi, emailsCount: this.sequence.emails?.length })` |
| `api-call` | `console.log('[WORKFLOW.sequences-suivi-detail-sauvegarder] API: PUT /api/sequences/' + this.sequence.id, { emails: this.sequence.emails?.length })` |
| `api-response` | `console.log('[WORKFLOW.sequences-suivi-detail-sauvegarder] API: Réponse reçue', { success: data.success, message: data.message })` |
| `state-updated` | `console.log('[WORKFLOW.sequences-suivi-detail-sauvegarder] STEP: hasChanges = false, saving = false')` |
| `toast-shown` | `console.log('[WORKFLOW.sequences-suivi-detail-sauvegarder] STEP: Toast confirmation affichée', { type: 'success', message: 'Séquence de suivi sauvegardée' })` |
| `end` | `console.log('[WORKFLOW.sequences-suivi-detail-sauvegarder] SUCCESS: Séquence sauvegardée en', duree, 'ms')` |
| `error` | `console.error('[WORKFLOW.sequences-suivi-detail-sauvegarder] ERROR:', error.message)` |
