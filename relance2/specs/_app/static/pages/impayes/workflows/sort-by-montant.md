...
|------------|------------------------|
| `start` | `console.log('[WORKFLOW.impayes-sort-by-montant] START: Tri par montant_total demandé, colonne actuelle =', this.sortColumn, 'direction =', this.sortDirection)` |
| `sort-applied` | `console.log('[WORKFLOW.impayes-sort-by-montant] STEP: sortColumn =', this.sortColumn, 'sortDirection =', this.sortDirection, 'currentPage reset à 1')` |
| `api-called` | `console.log('[WORKFLOW.impayes-sort-by-montant] API: GET /api/impayes?facture_soldee=false&sort=montant_total&order=' + this.sortDirection + '&limit=25&skip=0')` |
| `table-rerendered` | `console.log('[WORKFLOW.impayes-sort-by-montant] STEP: Tableau ré-rendu avec', this.impayes.length, 'lignes triées par montant_total')` |
| `end` | `console.log('[WORKFLOW.impayes-sort-by-montant] SUCCESS: Tri appliqué en', duree, 'ms —', this.impayes.length, 'résultats')` |
| `error` | `console.error('[WORKFLOW.impayes-sort-by-montant] ERROR:', error.message)` |
