...
db.query('impayes')
  ...
  .simplesort('reste_a_payer', { desc: true })
  .limit(25)
  .offset(0)
  .data();
```
...
|------------|------------------------|
| `start` | `console.log('[WORKFLOW.impayes-sort-by-reste] START: Tri par reste à payer demandé pour la colonne', column)` |
| `sort-applied` | `console.log('[WORKFLOW.impayes-sort-by-reste] STEP: sortColumn =', this.sortColumn, '/ sortDirection =', this.sortDirection, '/ currentPage réinitialisé à 1')` |
| `api-called` | `console.log('[WORKFLOW.impayes-sort-by-reste] STEP: Appel API GET /api/impayes avec sort=' + this.sortColumn + '&order=' + this.sortDirection)` |
| `table-rerendered` | `console.log('[WORKFLOW.impayes-sort-by-reste] DATA: Tableau rerendu avec', this.impayes.length, 'lignes triées')` |
| `end` | `console.log('[WORKFLOW.impayes-sort-by-reste] SUCCESS: Tri appliqué en', duree, 'ms (page', this.currentPage + ')')` |
| `error` | `console.error('[WORKFLOW.impayes-sort-by-reste] ERROR:', error)` |
