...
|------------|------------------------|
| `start` | `console.log('[WORKFLOW.impayes-save-note] START: Sauvegarde note pour impayé', impayeId, 'auteur', currentUser.name)` |
| `validation` | `console.log('[WORKFLOW.impayes-save-note] STEP: Validation du contenu de la note (longueur:', content.length, ')')` |
| `api-call` | `console.log('[WORKFLOW.impayes-save-note] API: PUT /api/impayes/' + impayeId, '- envoi de', updatedNotes.length, 'note(s)')` |
| `state-updated` | `console.log('[WORKFLOW.impayes-save-note] DATA: État local mis à jour, nouvelle note id:', newNote.id, '- total notes:', updatedNotes.length)` |
| `end` | `console.log('[WORKFLOW.impayes-save-note] SUCCESS: Note sauvegardée en', duree, 'ms')` |
| `error` | `console.error('[WORKFLOW.impayes-save-note] ERROR:', error.message)` |
