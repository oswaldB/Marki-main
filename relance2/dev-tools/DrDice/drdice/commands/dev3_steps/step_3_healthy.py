...
def step_3_healthy_test() -> tuple[bool, list[dict]]:
    ...
    Vérifie:
    - Frontend (screen healthy)
    - CouchDB
    - API /api/healthy avec validation JSON
    - Cron configuré pour appeler /api/healthy
    ...
    try:
        ...
        for test in tests:
            ...
            display = f"  {icon} {name}: HTTP {code}"
            if details:
                display += f" ({details})"
            console.print(display)
            
            # Afficher les checks détaillés si présents
            checks = test.get("checks", {})
            if checks:
                for check_name, check_ok in checks.items():
                    if isinstance(check_ok, bool):
                        check_icon = "[green]✓" if check_ok else "[red]✗"
                        console.print(f"      {check_icon} {check_name}")
