...
Les extensions suivantes sont autorisées selon le dossier :

- **`specs/`** : `.md` uniquement. Tout autre format est interdit.
- **`specs/LIRE_EN_PREMIER/`** : `.md`, `.sql` autorisés.
- **`specs/mockups/`** : `.html` uniquement. `.md`, `.js`, `.css` sont interdits.
- **`specs/wf-frontend/`** : `.md` uniquement. `.html`, `.js` sont interdits.
- **`specs/wf-backend/`** : `.md` uniquement. `.html`, `.js` sont interdits.
- **`specs/models/`** : `.md` uniquement. Tout autre format est interdit.
- **`specs/routes/`** : `.md` uniquement. Tout autre format est interdit.
...
Trois types de cells existent dans l'architecture :

- **Cell Écran** : Page web complète avec UI. Dossier : `app/screens/<nom>/`
- **Cell Backend** : API endpoint sans UI. Dossier : `app/backend-wf/<nom>/`
- **Cell Cron** : Tâche planifiée. Dossier : `app/cron/<nom>/`
...
Par couche technique, les technologies suivantes sont imposées ou interdites :

**Backend**
- Autorisé : Flask + Python
- Interdit : Django, FastAPI

**Frontend JS**
- Autorisé : Alpine.js 3.x
- Interdit : React, Vue, Angular, Vanilla JS

**Templating**
- Autorisé : Jinja2
- Interdit : React JSX, Vue SFC

**Styling**
- Autorisé : Tailwind CSS
- Interdit : Bootstrap, Material UI

**Database**
- Autorisé : SQLite3 (stdlib)
- Interdit : PostgreSQL, MySQL, **SQLAlchemy**, **Tout ORM**

**HTTP Client**
- Autorisé : Fetch API
- Interdit : Axios, jQuery
...
**ÉVITER ALPINE STORE.** On a très rarement besoin d'Alpine Store dans cette architecture.

- Si tu penses avoir besoin d'un Store → c'est probablement une mauvaise idée
- Privilégier les données locales dans `x-data` et la communication parent-enfant via props/events
- Le pattern Cell-based MVC avec des cells autonomes rend le Store inutile dans 99% des cas
- Exception valide : état global vraiment transverse (thème, auth user), mais encore une fois, réfléchis deux fois
...
<script>
    ...
    document.addEventListener("alpine:init", () => {
        Alpine.data("contactList", () => ({
            ...
            // 2. INIT
            {% include "workflows/init-contacts.html" %},

            // 3. WORKFLOWS MÉTIER
            {% include "workflows/load-contacts.html" %},
        }));
    });
</script>
```
...
+> rajoute que l'on a très rarement besoin de alpine store. Donc si tu veux faire du alpine store c'est probablement une mauvaise idée.
