...
class SidebarNavDual extends HTMLElement {
  constructor() {
    ...
    this.expanded = {
      ...
      contacts: this.activePage.startsWith('contacts')
    };
  }
  ...
  render() {
    ...
    <aside class="fixed left-0 top-0 h-screen z-[100] flex" 
           x-data="{ 
             activeRail: 'relance',
             expanded: { impayes: ${this.expanded.impayes}, relances: ${this.expanded.relances}, contacts: ${this.expanded.contacts} },
             isActive(page) {
               ...
               if (page === 'contacts' && current === 'contacts') return true;
               if (page === 'contacts-blacklist' && current === 'contacts-blacklist') return true;
               if (page === 'contacts-sans-email' && current === 'contacts-sans-email') return true;
               return false;
             }
           }"
           x-cloak>
      ...
      <div class="w-[220px] h-full bg-white border-r border-slate-200 flex flex-col">
        ...
        <nav class="flex-1 overflow-y-auto py-4">
          ...
          <!-- Contacts -->
          <div class="mx-2 mt-1">
            <button @click="expanded.contacts = !expanded.contacts"
                    :class="isActive('contacts') ? 'bg-sky-50 text-sky-700' : 'text-slate-600 hover:bg-slate-50'"
                    class="w-full flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-medium transition-all">
              <div class="flex items-center gap-3">
                <i class="fas fa-address-book w-5 text-center"></i>
                <span>Contacts</span>
              </div>
              <i class="fas fa-chevron-down text-xs transition-transform" :class="expanded.contacts ? 'rotate-180' : ''"></i>
            </button>
            
            <!-- Sous-menu Contacts -->
            <div x-show="expanded.contacts" x-collapse class="mt-1 ml-4 pl-4 border-l border-slate-200">
              <a href="/contacts" 
                 :class="isActive('contacts') ? 'text-sky-700' : 'text-slate-500 hover:text-slate-700'"
                 class="flex items-center gap-3 px-4 py-2 text-sm transition-all">
                <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                <span>Liste</span>
              </a>
              <a href="/contacts-blacklist" 
                 :class="isActive('contacts-blacklist') ? 'text-sky-700' : 'text-slate-500 hover:text-slate-700'"
                 class="flex items-center gap-3 px-4 py-2 text-sm transition-all">
                <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                <span>Blacklisté</span>
              </a>
              <a href="/contacts-sans-email" 
                 :class="isActive('contacts-sans-email') ? 'text-sky-700' : 'text-slate-500 hover:text-slate-700'"
                 class="flex items-center gap-3 px-4 py-2 text-sm transition-all">
                <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
