...
applySortAndPagination() {
  ...
  // Tri côté client avec localeCompare pour les strings
...
get filteredImpayes() {
  ...
  if (this.searchQuery) {
    ...
    result = result.filter(i => 
      i.numero_dossier?.toLowerCase().includes(q) ||
...
## Colonnes triables (côté client)

| Colonne | Champ | Type | Méthode de tri |
|---------|-------|------|-----------------|
| Numéro de dossier | `numero_dossier` | string | `localeCompare` |
| Numéro de facture | `nfacture` | string/number | `localeCompare` |
| Montant total | `montant_total` | number | `-` opérateur |
| Reste à payer | `reste_a_payer` | number | `-` opérateur |
| Date d'échéance | `date_echeance` | ISO date | `new Date()` |
| Nom du payeur | `payeur_nom` | string | `localeCompare` |

## Exemple de tri numérique

```javascript
// Pour les montants (number)
if (this.sortColumn === 'montant_total' || this.sortColumn === 'reste_a_payer') {
  sorted.sort((a, b) => {
...
// Pour les dates (ISO string)
if (this.sortColumn === 'date_echeance') {
  sorted.sort((a, b) => {
...
- **Alternative avec Mango Query :** Si utilisation de `pouchdb-find`, on peut créer un index et utiliser `sort` dans la requête
...
|--------|-------------|-----------------|
| Endpoint | `GET /api/impayes?sort=xxx&order=xxx` | Tri côté client avec `sort()` |
