# Workflow : Supprimer email suivi (PouchDB)
...
## Action
Supprimer un email de la séquence de suivi dans PouchDB
...
async supprimerEmail(index) {
  // 1. Confirm action
  if (!confirm('Supprimer cet email de suivi ?')) return;
  ...
  try {
    ...
    // 7. Notify
    this.toast('Email de suivi supprimé', 'success');
