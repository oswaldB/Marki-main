...
description: Charger le détail d'une séquence de relance depuis PouchDB avec ses étapes
...
# sequences-relance-detail-initial-load : Chargement initial Détail Séquence Relance (PouchDB)

## Description

Charger les détails complets d'une séquence de relance depuis PouchDB : étapes, modèles d'email, règles et aperçu.
...
/**
 * @action Extraire les règles d'attribution depuis la clé `regles_attribution`
...
```javascript
async loadSequenceDetail(sequenceId) {
  this.loading = true;
...
db.changes({
  ...
  console.error('Erreur sync séquence:', err);
});
```
