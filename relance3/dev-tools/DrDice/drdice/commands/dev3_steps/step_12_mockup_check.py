...
def step_12_check_mockup_similarity(cell_path: Path, cell_name: str, 
    ...
    # Similarité trop faible - Correction automatique
    console.print(f"\n[yellow]⚠️  Similarité trop faible ({similarity['total'] * 100:.1f}% < {min_similarity * 100:.0f}%)")
    console.print("  [blue]→ Régénération automatique de index.html...")
    
    # Générer le prompt pour correction
    prompt = _generate_fix_prompt_for_mockup(cell_path, cell_name, impl_content, mockup_content, similarity)
    
    # Sauvegarder le prompt pour debug
    prompt_file = cell_path / ".drdice" / "mockup-fix-prompt.txt"
    prompt_file.parent.mkdir(parents=True, exist_ok=True)
    prompt_file.write_text(prompt, encoding="utf-8")
    console.print(f"  [dim]  Prompt sauvegardé: {prompt_file}")
    
    # Exécution directe du prompt via pi -p
    console.print("  [blue]→ Exécution directe du prompt...")
    
    try:
        import subprocess
        
        # Appeler pi -p avec le prompt via stdin
